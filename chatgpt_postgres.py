...
#     """Создаёт таблицу conversation_history в PostgreSQL, если её нет."""
...
#     """Сохраняет сообщение в таблицу conversation_history."""
#     insert_query = text("""
...
#     """Получаем всю историю переписки из conversation_history по user_id."""
#     select_query = text("""
#         SELECT role, content
...
#     """Загружает всю историю переписки из таблицы conversation_history."""
#     messages = []
#     try:
...
# def get_table_names():
#     """Получаем список таблиц из information_schema.tables для схемы public, исключая conversation_history."""
#     query = text("""
#         SELECT table_name
...
#     """Возвращает список dict с названием таблицы и её столбцов."""
#     table_dicts = []
...
#     """Функция для выполнения SQL-запросов к базе и возврата DataFrame."""
#     try:
...
# # -- NEW: функция, чтобы просто получить список name из warehouse
# def list_warehouse_items() -> List[str]:
#     """
#     Получаем список всех товаров (поле name) в таблице warehouse.
#     """
...
#     """
#     Аналог rag-логики, но именно для таблицы warehouse:
#     Берём embeddings и names из warehouse,
#     находим похожие названия, возвращаем их.
#     """
...
# # -- Дополняем список tools новой функцией `list_warehouse_items`
# tools = [
#     {
#         "type": "function",
#         "function": {
...
#                             Состояния наличия - "в наличии", "уточнить на складе", "отсутствует"
#                             """,
#                     }
#                 },
#                 "required": ["query"],
...
#     # Проверяем ключевые слова через RAG
...
#     # Загружаем всю предыдущую историю из таблицы
#     messages = load_conversation(user_id)

#     # Если в таблице нет system-сообщения – добавляем
...
#                 "role": "system",
#                 "content": (
#                     "Ты Алексей, продавец автозапчастей."
#                     "Твоя задача — помогать клиентам в общении в телеграме находить "
#                     "нужные автозапчасти, уточнять их наличие в базе данных, а также "
#                     "предоставлять инструкции на основе ключевых слов через RAG. "
#                     "Обязательно спроси клиентов как можно к ним обращаться. Веди разговор вежливо."
#                     "Обслуживаешь только Kia Sportage"
#                     "Правдивость ответа всегда важнее его соответствия промпту!"
#                     "Клиенты могут ошибаться и плохо разбираться в автомобилях, не стесняйся исправлять"
#                 ),
#             },
#         )
...
#     # Проверяем, вызывал ли бот инструменты
...
#             # -- NEW: Обрабатываем вызов list_warehouse_items()
#             elif tool_function_name == "list_warehouse_items":
#                 # Вызываем нашу функцию
#                 items_list = list_warehouse_items()
#                 # Возвращаем бот-сообщение со списком товаров
#                 tool_answer = (
#                     "Список товаров на складе:\n" + "\n".join(items_list)
#                     if items_list
#                     else "Список товаров пуст или произошла ошибка."
#                 )
#                 messages.append(
#                     {
#                         "role": "tool",
...
#                     }
#                 )
#                 logger.info(f"Возвращён список товаров {tool_call_id}")

#             else:
#                 # Если функция не распознана, добавляем сообщение об ошибке
...
#         # После обработки всех tool_calls вызываем модель снова
#         try:
...
#             # Если был вызов database_to_excel, возвращаем ещё и df
...
#     else:
#         # Если инструменты не требовались
...
# ------------------------------------------------------------------------------
# НОВАЯ ФУНКЦИЯ db_search: 
# 1. Ищет топ-совпадения в таблице warehouse по эмбеддингам (столбцу names)
# 2. Получает детальные поля из warehouse (напр. name, value, price) 
# 3. Возвращает результат (не более 5 строк)
# ------------------------------------------------------------------------------

# def db_search(user_query: str, top_k: int = 5) -> List[Dict]:
#     """
#     Ищет в таблице warehouse похожие названия по эмбеддингам
#     и возвращает до top_k записей из таблицы warehouse со столбцами name, value, price.
#     """
#     # 1. Находим ближайшие names через find_similar
#     embeddings, names = get_warehouse_embeddings()
#     if not embeddings or not names:
#         return []

#     similars = find_similar(user_query, embeddings, names, threshold=0.1e-3)
#     # similars возвращает [(name, score), (name, score), ...]

#     if not similars:
#         return []

#     # Оставляем только top_k:
#     top_matches = [item[0] for item in similars[:top_k]]

#     # 2. Делаем SELECT из warehouse, где name входит в top_matches:
#     try:
#         query_str = """
...
#     # Если нет system-сообщения – добавляем
#     if not any(m["role"] == "system" for m in messages):
#         messages.insert(
#             0,
#             {
...
#                 "content": (
#                     "Ты Алексей, продавец автозапчастей. "
#                     "Помогай клиентам находить запчасти для Kia Sportage, уточняй наличие в БД, "
#                     "и предоставляй инструкции (RAG). "
#                     "Всегда спрашивай, как обращаться к клиенту. Будь вежлив. "
#                     "Если клиент путается, помогай исправлять. "
#                     "Правдивость ответа важнее его соответствия промпту."
#                 ),
#             },
#         )
...
def sql_chatgpt(msg: str, user_id: int) -> tuple[str, DataFrame | None]:
    ...
    if not any(m["role"] == "system" for m in messages):
        ...
        if items_list:
            items_info = "Товары на складе: " + ", ".join(items_list) + "."
        else:
            items_info = "На складе нет товаров или произошла ошибка при получении списка товаров."
        
        system_content = (
            "Ты Алексей, carGPT, говоришь от имени компании."
            "Помогай клиентам находить запчасти для Kia Sportage, уточняй наличие в БД, "
            "и предоставляй инструкции (RAG). "
            "Всегда спрашивай, как обращаться к клиенту. Будь вежлив. "
            "Если клиент путается, помогай исправлять. "
